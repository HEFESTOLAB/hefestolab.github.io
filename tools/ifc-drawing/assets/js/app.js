/*
 * HEFESTOLAB IFC Drawing · Revisión v0.9
 * ------------------------------------------------------------
...
(() => {
  ...
  const state = {
    ...
    modelCategoryColors: new Map(),
    modelItemColors: new Map(),
    busy: false,
    paperPPM: 1.7,
    sheetZoom: 1,
    demo: false,
  ...
  function setDrawTool(tool) {
    ...
    state.pendingAngleDimension = null;
    ...
    if (tool === 'diameter-dimension') toast('Cota de diámetro activa', 'Marca los dos extremos del diámetro y coloca la línea con el tercer clic.');
    if (tool === 'radius-dimension') toast('Cota de radio activa', 'Marca el centro, un punto de la circunferencia y coloca la línea con el tercer clic.');
    if (tool === 'angle-dimension') toast('Cota angular activa', 'Marca vértice, primer lado, segundo lado y un cuarto punto para fijar el arco.');
  ...
  const orientationDef = {
    ...
    // Oeste observa desde -X hacia +X; Este hace el recorrido contrario.
    // Mantener este sentido es esencial para que EdgeProjector clasifique
    // correctamente qué aristas quedan visibles y cuáles ocultas.
  ...
  function updateDrawingToolAvailability(d=activeDrawing()){
    ...
    if(locked&&state.drawTool!=='select'){state.drawTool='select';state.pendingDimension=null;state.pendingAngleDimension=null;state.pendingDimensionChain=null;state.pendingArea=null;$$('[data-draw-tool]').forEach(b=>b.classList.toggle('active',b.dataset.drawTool==='select'));}
  }
  ...
  function renderDrawing(){
    ...
    let preview='';
    if(state.pendingDimension?.a){ const p=state.pendingDimension; if(p.b) preview=annotationSvg({id:'preview',type:'dimension',dimensionKind:p.dimensionKind||'linear',a:p.a,b:p.b,offset:p.offset||0},d,true); else preview=`<circle cx="${p.a[0]}" cy="${p.a[1]}" r="${Math.max(vb.w/500,.01)}" fill="#2563eb"/>`; }
    if(state.pendingAngleDimension?.vertex){const p=state.pendingAngleDimension,r=Math.max(vb.w/500,.01);if(p.a&&p.b){const cursor=p.cursor||p.b,radius=Math.max(.05,Math.hypot(cursor[0]-p.vertex[0],cursor[1]-p.vertex[1]));preview+=angleDimensionSvg({id:'preview-angle',type:'angleDimension',vertex:p.vertex,a:p.a,b:p.b,radius},d,true);}else{preview+=`<circle cx="${p.vertex[0]}" cy="${p.vertex[1]}" r="${r}" fill="#2563eb"/>${p.a?`<line class="dim-line" x1="${p.vertex[0]}" y1="${p.vertex[1]}" x2="${p.a[0]}" y2="${p.a[1]}"/>`:''}`;}}
  ...
  function dimensionKind(a){return a?.dimensionKind||'linear';}
  function isAlignedDimensionTool(tool){return ['dimension','diameter-dimension','radius-dimension'].includes(tool);}
  function dimensionLabel(a,g=dimensionGeom(a)){
    if(a.textOverride)return a.textOverride;
    const value=formatLength(g.L),kind=dimensionKind(a);
    return kind==='diameter'?`Ø ${value}`:kind==='radius'?`R ${value}`:value;
  }
  function angleDimensionGeom(a){return dimensionTools.angleGeometry(a);}
  function angleDimensionLabel(a,g=angleDimensionGeom(a)){return a.textOverride||`${g.degrees.toFixed(g.degrees<10?2:1).replace('.',',')}°`;}
  function angleDimensionSvg(a,d,preview=false){
    const g=angleDimensionGeom(a),b=d.bounds||ensureDrawingBounds(d),fs=Math.max(.16,Math.max(b.width,b.height)/100),tick=fs*.52,points=dimensionTools.angleArcPoints(g),text=dimensionTools.angleTextLayout(g,fs*.75),selected=state.selectedAnnotationId===a.id,sel=!preview&&selected?' annotation-selected':'',tangent=(angle)=>[-Math.sin(angle)*tick,Math.cos(angle)*tick],t1=tangent(g.startAngle),t2=tangent(g.endAngle);
    return `<g class="${sel.trim()}" data-ann="${a.id}"><line class="dim-ext" x1="${g.vertex[0]}" y1="${g.vertex[1]}" x2="${g.start[0]}" y2="${g.start[1]}"/><line class="dim-ext" x1="${g.vertex[0]}" y1="${g.vertex[1]}" x2="${g.end[0]}" y2="${g.end[1]}"/><polyline class="dim-line" points="${points.map(p=>p.join(',')).join(' ')}"/><line class="dim-line" x1="${g.start[0]-t1[0]}" y1="${g.start[1]-t1[1]}" x2="${g.start[0]+t1[0]}" y2="${g.start[1]+t1[1]}"/><line class="dim-line" x1="${g.end[0]-t2[0]}" y1="${g.end[1]-t2[1]}" x2="${g.end[0]+t2[0]}" y2="${g.end[1]+t2[1]}"/><polyline class="dim-hit" data-ann="${a.id}" points="${points.map(p=>p.join(',')).join(' ')}"/><text class="dim-text" data-ann="${a.id}" x="${text.x}" y="${text.y}" font-size="${fs}" text-anchor="middle">${esc(angleDimensionLabel(a,g))}</text></g>`;
  }
  function annotationSvg(a,d,preview=false){
    ...
    if(a.type==='angleDimension')return angleDimensionSvg(a,d,preview);
    if(a.type==='dimension'){
      const g=dimensionGeom(a), b=d.bounds||ensureDrawingBounds(d), fs=Math.max(.16,Math.max(b.width,b.height)/100), tick=fs*.52;
      const label=esc(dimensionLabel(a,g)),text=dimensionTools.textLayout(g,fs*.4);
  ...
  function getSnap(p,d,options={}){
    if(!state.snapEnabled)return {p:[...p],type:null,distance:Infinity};const idx=buildSnapIndex(d),tol=Number.isFinite(+options.tolerance)?Math.max(1e-9,+options.tolerance):worldTolerance(d,12);let best={p:[...p],type:null,distance:tol};
  ...
  async function onDrawingClick(evt){
    ...
    if(state.drawTool==='angle-dimension'){
      const snap=getSnap(raw,d),p=snap.type?snap.p:raw;state.pendingAngleDimension||={vertex:null,a:null,b:null,cursor:null};const pending=state.pendingAngleDimension;
      if(!pending.vertex){pending.vertex=[...p];toast('Cota angular · vértice',`Referencia: ${snap.type||'libre'}. Marca el primer lado.`);}
      else if(!pending.a){if(Math.hypot(p[0]-pending.vertex[0],p[1]-pending.vertex[1])<1e-6){toast('Referencia no válida','El primer lado debe separarse del vértice.','warn');return;}pending.a=[...p];toast('Cota angular · lado 1','Marca un punto del segundo lado.');}
      else if(!pending.b){if(Math.hypot(p[0]-pending.vertex[0],p[1]-pending.vertex[1])<1e-6){toast('Referencia no válida','El segundo lado debe separarse del vértice.','warn');return;}pending.b=[...p];toast('Cota angular · lados definidos','Cuarto clic para fijar el radio del arco.');}
      else{const radius=Math.max(.05,Math.hypot(raw[0]-pending.vertex[0],raw[1]-pending.vertex[1])),ann={id:uid('angle'),type:'angleDimension',vertex:[...pending.vertex],a:[...pending.a],b:[...pending.b],radius},g=angleDimensionGeom(ann);if(g.degrees<.05){toast('Ángulo no válido','Los dos lados deben definir un ángulo apreciable.','warn');return;}d.annotations.push(ann);state.selectedAnnotationId=ann.id;state.pendingAngleDimension=null;state.snapHover=null;toast('Cota angular creada',angleDimensionLabel(ann,g),'good');}
      renderDrawing();renderInspector();renderSheet();return;
    }
    const aligned=isAlignedDimensionTool(state.drawTool),base=aligned&&state.pendingDimension&&!state.pendingDimension.b?state.pendingDimension.a:null,res=resolveDrawingPoint(raw,d,base,evt),p=res.p;
    if(aligned){
      const kind=state.drawTool==='diameter-dimension'?'diameter':state.drawTool==='radius-dimension'?'radius':'linear';
      if(!state.pendingDimension){ state.pendingDimension={a:p,dimensionKind:kind}; toast(kind==='diameter'?'Diámetro · extremo 1':kind==='radius'?'Radio · centro':'Cota · punto 1',`Referencia: ${res.type||'libre'}. Selecciona el segundo punto.`); }
        ...
        const ann={id:uid('dim'),type:'dimension',dimensionKind:state.pendingDimension.dimensionKind||kind,a:[...a],b:[...b],offset:off,axis:state.pendingDimension.axis||null}; d.annotations.push(ann);state.selectedAnnotationId=ann.id;state.pendingDimension=null;state.snapHover=null;toast(kind==='diameter'?'Cota de diámetro creada':kind==='radius'?'Cota de radio creada':'Cota creada',dimensionLabel(ann),'good');
      }
      renderDrawing();renderInspector();renderSheet();return;
    }
  ...
  async function addViewToSheet(drawingId,drop=null){
    ...
    const vp={id:uid('vp'),drawingId:d.id,x,y,w,h,scale,cx:d.bounds.cx,cy:d.bounds.cy,...(d.kind==='threeD'?{imageFit:'cover',imageFocusX:50,imageFocusY:50}:{})};
  ...
  function viewportControlsMarkup(){return '<i class="vp-resize" title="Redimensionar desde la esquina"></i><i class="vp-crop vp-crop-left" data-crop-side="left" title="Recortar desde la izquierda"></i><i class="vp-crop vp-crop-right" data-crop-side="right" title="Recortar desde la derecha"></i><i class="vp-crop vp-crop-top" data-crop-side="top" title="Recortar desde arriba"></i><i class="vp-crop vp-crop-bottom" data-crop-side="bottom" title="Recortar desde abajo"></i>';}
  ...
  function makeViewportElement(vp,ppm){
    ...
    if(d.kind==='threeD'){
      const fit=vp.imageFit==='contain'?'contain':'cover',focusX=clamp(+vp.imageFocusX||50,0,100),focusY=clamp(+vp.imageFocusY||50,0,100);div.classList.add('sheet-vp-3d');div.innerHTML=`<div class="vp-image-wrap"><img src="${d.imageData}" alt="" style="object-fit:${fit};object-position:${focusX}% ${focusY}%"></div>${viewportLabelMarkup(vp,d,ppm)}${viewportControlsMarkup()}`;
    }else if(d.kind==='areaSchedule'){
      div.classList.add('sheet-vp-area-schedule');div.innerHTML=`${areaScheduleTable(d)}${viewportLabelMarkup(vp,d,ppm)}${viewportControlsMarkup()}`;
    }else{
      ...
      div.innerHTML=`<svg viewBox="${win.x} ${win.y} ${win.w} ${win.h}" preserveAspectRatio="none">${hid}${vis}${ann}</svg>${viewportLabelMarkup(vp,d,ppm)}${viewportControlsMarkup()}`;
    }
    div.addEventListener('pointerdown',e=>{
      e.stopPropagation();state.selectedViewportId=vp.id;renderInspector();
      const movingLabel=!!e.target.closest?.('[data-vp-label]'),resizing=e.target.classList.contains('vp-resize'),cropSide=e.target.dataset?.cropSide||null,label=viewportLabelOffset(vp),start={x:e.clientX,y:e.clientY,vx:vp.x,vy:vp.y,vw:vp.w,vh:vp.h,cx:vp.cx,cy:vp.cy,labelX:label.x,labelY:label.y};div.setPointerCapture?.(e.pointerId);if(movingLabel)div.classList.add('moving-label');
      const move=ev=>{const dx=(ev.clientX-start.x)/ppm,dy=(ev.clientY-start.y)/ppm;const sh=activeSheet(),[pw,ph]=sheetSize(sh),mmPerUnit=isNonScaleDrawing(d)?null:viewportWindow({...vp,w:start.vw,h:start.vh,cx:start.cx,cy:start.cy},d).mmPerUnit;if(movingLabel){vp.labelDx=clamp(vp.x+start.labelX+dx,10,pw-10)-vp.x;vp.labelDy=clamp(vp.y+start.labelY+dy,10,ph-10)-vp.y;}else if(cropSide==='left'){const x=clamp(start.vx+dx,10,start.vx+start.vw-35),delta=x-start.vx;vp.x=x;vp.w=start.vw-delta;if(mmPerUnit)vp.cx=start.cx+delta/(2*mmPerUnit);}else if(cropSide==='right'){vp.w=clamp(start.vw+dx,35,pw-start.vx-10);if(mmPerUnit)vp.cx=start.cx+(vp.w-start.vw)/(2*mmPerUnit);}else if(cropSide==='top'){const y=clamp(start.vy+dy,10,start.vy+start.vh-28),delta=y-start.vy;vp.y=y;vp.h=start.vh-delta;if(mmPerUnit)vp.cy=start.cy+delta/(2*mmPerUnit);}else if(cropSide==='bottom'){vp.h=clamp(start.vh+dy,28,ph-start.vy-10);if(mmPerUnit)vp.cy=start.cy+(vp.h-start.vh)/(2*mmPerUnit);}else if(resizing){vp.w=clamp(start.vw+dx,35,pw-vp.x-10);vp.h=clamp(start.vh+dy,28,ph-vp.y-10);}else{vp.x=clamp(start.vx+dx,10,pw-vp.w-10);vp.y=clamp(start.vy+dy,10,ph-vp.h-10);}renderViewportLive(div,vp,d,ppm);};
  ...
  function renderViewportLive(div,vp,d,ppm){ div.style.left=`${vp.x*ppm}px`;div.style.top=`${vp.y*ppm}px`;div.style.width=`${vp.w*ppm}px`;div.style.height=`${vp.h*ppm}px`;const label=viewportLabelOffset(vp),labelEl=div.querySelector('[data-vp-label]');if(labelEl){labelEl.style.left=`${label.x*ppm}px`;labelEl.style.top=`${label.y*ppm}px`;}if(!isNonScaleDrawing(d)){const win=viewportWindow(vp,d);div.querySelector('svg')?.setAttribute('viewBox',`${win.x} ${win.y} ${win.w} ${win.h}`);}else if(d?.kind==='threeD'){const img=div.querySelector('img');if(img){img.style.objectFit=vp.imageFit==='contain'?'contain':'cover';img.style.objectPosition=`${clamp(+vp.imageFocusX||50,0,100)}% ${clamp(+vp.imageFocusY||50,0,100)}%`;}} }
  function sheetAnnotationSvg(a,d,vp){
    if(a.type==='angleDimension'){
      const g=angleDimensionGeom(a),win=viewportWindow(vp,d),fs=DIMENSION_TEXT_MM/Math.max(.001,win.mmPerUnit),points=dimensionTools.angleArcPoints(g),text=dimensionTools.angleTextLayout(g,fs*.75);
      return `<g><line class="vp-visible" x1="${g.vertex[0]}" y1="${g.vertex[1]}" x2="${g.start[0]}" y2="${g.start[1]}"/><line class="vp-visible" x1="${g.vertex[0]}" y1="${g.vertex[1]}" x2="${g.end[0]}" y2="${g.end[1]}"/><polyline class="vp-visible" fill="none" points="${points.map(p=>p.join(',')).join(' ')}"/><text x="${text.x}" y="${text.y}" font-size="${fs}" text-anchor="middle" fill="#1e3a8a">${esc(angleDimensionLabel(a,g))}</text></g>`;
    }
    if(a.type==='dimension'){
      ...
      return `<g><line class="vp-visible" x1="${a.a[0]}" y1="${a.a[1]}" x2="${g.q1[0]}" y2="${g.q1[1]}"/><line class="vp-visible" x1="${a.b[0]}" y1="${a.b[1]}" x2="${g.q2[0]}" y2="${g.q2[1]}"/><line class="vp-visible" x1="${g.q1[0]}" y1="${g.q1[1]}" x2="${g.q2[0]}" y2="${g.q2[1]}"/><text x="${text.x}" y="${text.y}" font-size="${fs}" text-anchor="middle" fill="#1e3a8a" transform="rotate(${text.angle} ${text.x} ${text.y})">${esc(dimensionLabel(a,g))}</text></g>`;
    }
  ...
  function pdfLineClipped(doc,a,b,vp){const c=clipLine(a[0],a[1],b[0],b[1],vp.x,vp.y,vp.x+vp.w,vp.y+vp.h);if(!c)return false;doc.line(c[0],c[1],c[2],c[3]);return true;}
  ...
  function pdfDrawAnnotations(doc,d,vp){
    ...
    for(const a of d.annotations||[]){
      if(a.type==='angleDimension'){
        const g=angleDimensionGeom(a),win=viewportWindow(vp,d),modelText=DIMENSION_TEXT_MM/Math.max(.001,win.mmPerUnit),layout=dimensionTools.angleTextLayout(g,modelText*.75),vertex=pdfMap(vp,d,...g.vertex),start=pdfMap(vp,d,...g.start),end=pdfMap(vp,d,...g.end),points=dimensionTools.angleArcPoints(g).map(p=>pdfMap(vp,d,...p)),text=pdfMap(vp,d,layout.x,layout.y);pdfLineClipped(doc,vertex,start,vp);pdfLineClipped(doc,vertex,end,vp);for(let i=0;i<points.length-1;i++)pdfLineClipped(doc,points[i],points[i+1],vp);if(text[0]>=vp.x&&text[0]<=vp.x+vp.w&&text[1]>=vp.y&&text[1]<=vp.y+vp.h){doc.setFontSize(DIMENSION_TEXT_POINTS);doc.text(angleDimensionLabel(a,g),text[0],text[1],{align:'center',baseline:'alphabetic'});}
        ...
        doc.setFontSize(DIMENSION_TEXT_POINTS);const visible=pdfLineClipped(doc,q1,q2,vp);pdfLineClipped(doc,p1,q1,vp);pdfLineClipped(doc,p2,q2,vp);if(visible){const tx=g.nx*.6,ty=g.ny*.6;pdfLineClipped(doc,[q1[0]-tx,q1[1]-ty],[q1[0]+tx,q1[1]+ty],vp);pdfLineClipped(doc,[q2[0]-tx,q2[1]-ty],[q2[0]+tx,q2[1]+ty],vp);if(text[0]>=vp.x&&text[0]<=vp.x+vp.w&&text[1]>=vp.y&&text[1]<=vp.y+vp.h)doc.text(dimensionLabel(a,g),text[0],text[1],{align:'center',angle:-layout.angle,baseline:'alphabetic'});}
  ...
  async function pdfDraw3DView(doc,d,vp){
    if(!d?.imageData)return;const aspect=d.imageAspect||((d.imageWidth||16)/Math.max(1,d.imageHeight||9));doc.setFillColor(255,255,255);doc.rect(vp.x,vp.y,vp.w,vp.h,'F');
    if(vp.imageFit==='contain'){let w=vp.w,h=w/aspect;if(h>vp.h){h=vp.h;w=h*aspect;}const x=vp.x+(vp.w-w)/2,y=vp.y+(vp.h-h)/2;doc.addImage(d.imageData,d.imageFormat||'JPEG',x,y,w,h,undefined,'FAST');}
    else{const img=new Image();img.src=d.imageData;await new Promise((resolve,reject)=>{if(img.complete&&img.naturalWidth)return resolve();img.onload=resolve;img.onerror=()=>reject(new Error('No se pudo preparar el recorte de la vista 3D.'));});const sw=img.naturalWidth||d.imageWidth||1600,sh=img.naturalHeight||d.imageHeight||900,target=vp.w/Math.max(.001,vp.h),source=sw/Math.max(1,sh),fx=clamp((+vp.imageFocusX||50)/100,0,1),fy=clamp((+vp.imageFocusY||50)/100,0,1);let sx=0,sy=0,cw=sw,ch=sh;if(source>target){cw=sh*target;sx=(sw-cw)*fx;}else{ch=sw/target;sy=(sh-ch)*fy;}const canvas=document.createElement('canvas'),max=1800;if(target>=1){canvas.width=max;canvas.height=Math.max(1,Math.round(max/target));}else{canvas.height=max;canvas.width=Math.max(1,Math.round(max*target));}canvas.getContext('2d').drawImage(img,sx,sy,cw,ch,0,0,canvas.width,canvas.height);doc.addImage(canvas.toDataURL('image/jpeg',.94),'JPEG',vp.x,vp.y,vp.w,vp.h,undefined,'FAST');}
    doc.setDrawColor(180,188,198);doc.setLineWidth(.12);doc.rect(vp.x,vp.y,vp.w,vp.h);
  }
  ...
  function exportDxf(){
    ...
    for(const a of d.annotations||[]){if(a.type==='angleDimension'){const g=angleDimensionGeom(a),pts=dimensionTools.angleArcPoints(g),text=dimensionTools.angleTextLayout(g,.24);e+=dxfLine('DIMENSIONS',[g.vertex[0],g.vertex[1],g.start[0],g.start[1]])+dxfLine('DIMENSIONS',[g.vertex[0],g.vertex[1],g.end[0],g.end[1]]);for(let i=0;i<pts.length-1;i++)e+=dxfLine('DIMENSIONS',[pts[i][0],pts[i][1],pts[i+1][0],pts[i+1][1]]);e+=`0\nTEXT\n8\nDIMENSIONS\n10\n${text.x}\n20\n${-text.y}\n30\n0\n40\n0.2\n1\n${angleDimensionLabel(a,g)}\n`;}else if(a.type==='dimension'){const g=dimensionGeom(a);e+=dxfLine('DIMENSIONS',[a.a[0],a.a[1],g.q1[0],g.q1[1]])+dxfLine('DIMENSIONS',[a.b[0],a.b[1],g.q2[0],g.q2[1]])+dxfLine('DIMENSIONS',[g.q1[0],g.q1[1],g.q2[0],g.q2[1]]);e+=`0\nTEXT\n8\nDIMENSIONS\n10\n${g.mid[0]}\n20\n${-g.mid[1]}\n30\n0\n40\n0.2\n1\n${dimensionLabel(a,g)}\n`;}else if(a.type==='text'){e+=`0\nTEXT\n8\nANNOTATION\n10\n${a.p[0]}\n20\n${-a.p[1]}\n30\n0\n40\n0.22\n1\n${a.text.replace(/[\r\n]+/g,' ')}\n`;}else if(a.type==='area'){ensureAreaAnnotation(a);for(let i=0;i<a.polygon.length;i++){const p=a.polygon[i],q=a.polygon[(i+1)%a.polygon.length];e+=dxfLine('AREAS',[p[0],p[1],q[0],q[1]],'#0284c7');}if(areaLabelVisible(a,d))e+=`0\nTEXT\n8\nAREAS\n10\n${a.center[0]}\n20\n${-a.center[1]}\n30\n0\n40\n0.22\n1\n${String(a.number?`${a.number} - ${a.name}`:a.name).replace(/[\r\n]+/g,' ')} ${fmt(a.areaM2,2)} m2\n`;}}
  ...
  function serializeProject(){
    return {schema:PROJECT_SCHEMA,version:PROJECT_VERSION,savedAt:new Date().toISOString(),application:'HEFESTO IFC Drawing',projectName:state.projectName,source:{name:state.source,meta:state.sourceMeta},levels:state.levels,drawings:state.drawings.map(drawingToJson),sheets:state.sheets,activeDrawingId:state.activeDrawingId,activeSheetId:state.activeSheetId,viewBoxes:[...state.viewBoxes.entries()],modelDisplay:{categoryColors:[...state.modelCategoryColors],itemColors:[...state.modelItemColors]},preferences:{hiddenLines:!!el.toggleHidden.checked},note:'El IFC original no se incrusta. Usa Reconectar IFC al reabrir para continuar creando vistas. Las vistas vectoriales, planos, áreas, cuadros, colores 3D y cotas sí quedan guardados.'};
  }
  ...
  async function applyProject(data,fileName='proyecto',options={}){assertProjectData(data);await clearIfcSelection();if(state.engine)for(const [modelId] of [...state.engine.fragments.list]){try{state.engine.fragments.core.disposeModel(modelId);}catch(_){ }}
    const asDemo=!!options.demo;state.modelLoaded=false;state.modelBox=null;state.modelBoxSource=null;state.activeModelId=null;state.ifcModels=0;state.geometryMap={};state.ifcCategories=[];state.categoryItems=new Map();state.itemCategories=new Map();state.modelCategoryColors=new Map(data.modelDisplay?.categoryColors||[]);state.modelItemColors=new Map(data.modelDisplay?.itemColors||[]);state.demo=asDemo;state.source=data.source?.name||fileName;state.sourceMeta=data.source?.meta||{schema:'PROYECTO',size:0,storeys:[]};state.projectName=String(data.projectName||safeName(fileName)||'Proyecto HEFESTO');state.levels=data.levels.map(l=>({...l}));state.drawings=[];setProjectDetached(!asDemo);
    ...
    state.sheets=data.sheets.map(sh=>({...sh,viewports:Array.isArray(sh.viewports)?sh.viewports.map(vp=>{const out={...vp},drawing=state.drawings.find(d=>d.id===out.drawingId);if(drawing?.kind==='threeD'){out.imageFit=out.imageFit||'cover';if(!Number.isFinite(+out.imageFocusX))out.imageFocusX=50;if(!Number.isFinite(+out.imageFocusY))out.imageFocusY=50;}return out;}):[]}));state.viewBoxes=new Map((data.viewBoxes||[]).filter(pair=>Array.isArray(pair)&&pair.length===2&&pair[1]&&Number.isFinite(+pair[1].w)&&Number.isFinite(+pair[1].h)));state.activeDrawingId=state.drawings.some(d=>d.id===data.activeDrawingId)?data.activeDrawingId:(state.drawings[0]?.id||null);state.activeSheetId=state.sheets.some(sh=>sh.id===data.activeSheetId)?data.activeSheetId:(state.sheets[0]?.id||null);state.selectedViewportId=null;state.selectedAnnotationId=null;state.pendingDimension=null;state.pendingAngleDimension=null;state.pendingDimensionChain=null;state.pendingArea=null;state.sheetZoom=1;el.toggleHidden.checked=data.preferences?.hiddenLines!==false;el.projectName.textContent=state.projectName;el.fileMeta.textContent=asDemo?`Demo técnica · ${state.drawings.length} vistas · ${state.sheets.length} plano${state.sheets.length===1?'':'s'}`:`Proyecto HEFESTO · ${state.drawings.length} vistas · ${state.sheets.length} planos`;el.modelBadge.textContent=asDemo?'DEMO TÉCNICA · IFC NO INCRUSTADO':'PROYECTO GUARDADO · IFC NO INCRUSTADO';el.engineHint.textContent=asDemo?'Vistas, cotas y plano editables · abre el IFC para nuevas proyecciones':'Documentación reabierta · usa Reconectar IFC para continuar';el.start.classList.add('hidden');renderTrees();if(state.activeSheetId)setMode('sheet');else if(state.activeDrawingId)await activateDrawing(state.activeDrawingId);else setMode('model');renderDrawing();renderSheet();renderInspector();setStatus(asDemo?'Demo técnica lista':'Proyecto reabierto · IFC pendiente','ok');toast(asDemo?'Demo cargada':'Proyecto reabierto',asDemo?`${state.projectName} · ${state.drawings.length} vistas · ${state.sheets.length} plano${state.sheets.length===1?'':'s'}.`:`${state.drawings.length} vistas · ${state.sheets.length} planos · reconecta ${state.source||'el IFC original'} para continuar.`,'good');
  }
  ...
  async function loadDemo(){
    if(state.busy)return;if(location.protocol==='file:'){showModal({title:'Demo técnica: abrir mediante servidor local',html:'<div class="modal-note">La nueva demo es un proyecto técnico completo de 2,7 MB. Ejecuta <b>INICIAR_HEFESTOLAB_LOCAL.bat</b> en la raíz de la copia local y ábrela por <b>http://127.0.0.1</b>.</div>',actions:[{label:'Entendido',primary:true,onClick:closeModal}]});return;}
    showProgress('Cargando demo técnica','V_3 · acumulador OPUS5 alto',.08,'proyecto');try{const response=await fetch('assets/data/demo-project.json?v=1.0.0',{cache:'no-store'});if(!response.ok)throw new Error(`No se pudo leer la demo (${response.status}).`);updateProgress('Interpretando vistas, cotas, 3D y plano',.55,'proyecto');const data=await response.json();await applyProject(data,'demo-project.json',{demo:true});hideProgress('Demo técnica cargada');}catch(err){console.error(err);clearInterval(state.progressState.timer);state.progressState.timer=null;el.progress.classList.add('hidden');state.busy=false;setStatus('Error al cargar la demo','error');toast('No se pudo cargar la demo técnica',err.message||String(err),'bad');}
  }
  ...
  function fragmentColor(hex){const rgb=hexRgb(hex)||[37,99,235];return {isColor:true,r:rgb[0]/255,g:rgb[1]/255,b:rgb[2]/255};}
  function resolvedModelColor(modelId,localId,category=null){return state.modelItemColors.get(itemKey(modelId,localId))||state.modelCategoryColors.get(category||state.itemCategories.get(itemKey(modelId,localId)))||null;}
  async function highlightModelItems(modelId,ids,color){const model=modelById(modelId),list=[...(ids||[])];if(!model||!list.length)return;await model.highlight?.(list,{color:fragmentColor(color),renderedFaces:1,opacity:1,transparent:false});}
  async function restoreModelItemColor(modelId,localId,category=null){const model=modelById(modelId);if(!model)return;try{await model.resetHighlight?.([localId]);}catch(_){ }const color=resolvedModelColor(modelId,localId,category);if(color)try{await highlightModelItems(modelId,[localId],color);}catch(err){console.warn('Color 3D:',err);}}
  async function restoreAllModelColors(){for(const [category,color] of state.modelCategoryColors){const group=state.categoryItems.get(category);if(!group)continue;for(const [modelId,ids] of Object.entries(group.map||{}))try{await highlightModelItems(modelId,ids,color);}catch(err){console.warn('Color de categoría 3D:',err);}}for(const [key,color] of state.modelItemColors){const cut=key.lastIndexOf(':'),modelId=key.slice(0,cut),rawId=key.slice(cut+1),allowed=state.geometryMap[modelId],localId=allowed&&[...allowed].find(id=>String(id)===rawId);if(localId!==undefined)try{await highlightModelItems(modelId,[localId],color);}catch(err){console.warn('Color de elemento 3D:',err);}}await refreshFragmentsAfterCamera();}
  async function applySelectedModelColor(color){const s=state.selectedIfc;if(!s)return;state.modelItemColors.set(s.key,color);await highlightModelItems(s.modelId,[s.localId],color);await refreshFragmentsAfterCamera();renderInspector();toast('Color 3D aplicado',`${String(s.category).replace(/^IFC/,'')} · #${s.localId}`,'good');}
  async function clearSelectedModelColor(){const s=state.selectedIfc;if(!s)return;state.modelItemColors.delete(s.key);await restoreModelItemColor(s.modelId,s.localId,s.category);await refreshFragmentsAfterCamera();renderInspector();toast('Color 3D restablecido','Se conserva el color de categoría si existe.','good');}
  async function applySelectedCategoryModelColor(color){const s=state.selectedIfc,group=s&&state.categoryItems.get(s.category);if(!s||!group)return;state.modelCategoryColors.set(s.category,color);for(const [modelId,ids] of Object.entries(group.map||{}))await highlightModelItems(modelId,ids,color);for(const [key,itemColor] of state.modelItemColors){const cut=key.lastIndexOf(':'),modelId=key.slice(0,cut),rawId=key.slice(cut+1),ids=group.map?.[modelId],localId=ids&&[...ids].find(id=>String(id)===rawId);if(localId!==undefined)await highlightModelItems(modelId,[localId],itemColor);}await refreshFragmentsAfterCamera();renderInspector();toast('Color 3D de categoría aplicado',String(s.category).replace(/^IFC/,''),'good');}
  async function clearSelectedCategoryModelColor(){const s=state.selectedIfc,group=s&&state.categoryItems.get(s.category);if(!s||!group)return;state.modelCategoryColors.delete(s.category);for(const [modelId,ids] of Object.entries(group.map||{})){const model=modelById(modelId);try{await model?.resetHighlight?.([...ids]);}catch(_){ }}await restoreAllModelColors();renderInspector();toast('Color 3D de categoría restablecido',String(s.category).replace(/^IFC/,''),'good');}
  ...
  async function clearIfcSelection(){
    const prev=state.selectedIfc;if(prev)await restoreModelItemColor(prev.modelId,prev.localId,prev.category);
  ...
  async function buildIfcSemanticIndex(){
    ...
    state.ifcCategories=[...state.categoryItems.values()].sort((a,b)=>a.name.localeCompare(b.name,'es'));await restoreAllModelColors();
  }
  ...
  function renderIfcSelectionInspector(selection,d){
    const target=d&&!d.pending&&!isNonScaleDrawing(d)?d:null,itemColor=state.modelItemColors.get(selection.key),categoryColor=state.modelCategoryColors.get(selection.category),color=itemColor||categoryColor||categoryDefaultColor(selection.category);el.inspectorTitle.textContent='Elemento IFC';
    el.inspector.innerHTML=`<div class="prop-section"><div class="prop-head">SELECCIÓN 3D EXACTA</div><div class="prop-body"><div class="field"><label>Categoría</label><input value="${esc(String(selection.category).replace(/^IFC/,''))}" readonly></div><div class="field"><label>GlobalId</label><input value="${esc(selection.guid||'No disponible')}" readonly></div><div class="field"><label>ID local</label><input value="${esc(selection.localId)}" readonly></div><div class="field"><label>Vista documental activa</label><input value="${esc(target?.name||'Ninguna vista 2D generada')}" readonly></div></div></div><div class="prop-section"><div class="prop-head">COLOR EN MODELO 3D</div><div class="prop-body"><div class="field"><label>Color</label><input id="ifc3dColor" type="color" value="${esc(color)}"></div><div class="prop-buttons"><button class="prop-btn primary" id="ifc3dApplyItem">Aplicar al elemento</button><button class="prop-btn" id="ifc3dClearItem" ${itemColor?'':'disabled'}>Original elemento</button><button class="prop-btn primary" id="ifc3dApplyCategory">Aplicar categoría</button><button class="prop-btn" id="ifc3dClearCategory" ${categoryColor?'':'disabled'}>Original categoría</button></div><p class="drag-hint">Los colores se conservan al capturar vistas 3D y al guardar el proyecto.</p></div></div>${target?selectedIfcBlock(target):'<p class="drag-hint">Abre o genera una vista 2D para aplicar además visibilidad o color documental.</p>'}<div class="prop-section"><div class="prop-head">ACCIONES</div><div class="prop-body"><div class="prop-buttons"><button class="prop-btn" id="ifcClearSelection">Cerrar selección</button>${target?'<button class="prop-btn primary" id="ifcOpenDrawing">Volver a la vista 2D</button>':''}</div></div></div>`;
    bindSelectedIfcActions(target,selection);$('#ifc3dApplyItem')?.addEventListener('click',()=>applySelectedModelColor($('#ifc3dColor')?.value||color));$('#ifc3dClearItem')?.addEventListener('click',clearSelectedModelColor);$('#ifc3dApplyCategory')?.addEventListener('click',()=>applySelectedCategoryModelColor($('#ifc3dColor')?.value||color));$('#ifc3dClearCategory')?.addEventListener('click',clearSelectedCategoryModelColor);$('#ifcClearSelection')?.addEventListener('click',async()=>{await clearIfcSelection();renderInspector();refreshFragmentsAfterCamera();});$('#ifcOpenDrawing')?.addEventListener('click',()=>activateDrawing(target.id));
  }
  ...
  function renderAnnotationInspector(a,d){
    const kind=dimensionKind(a),kindName=kind==='diameter'?'Diámetro':kind==='radius'?'Radio':'Lineal';
    el.inspectorTitle.textContent=a.type==='angleDimension'?'Cota angular':a.type==='dimension'?(a.chainId?'Cadena de cotas':`Cota ${kindName.toLowerCase()}`):a.type==='area'?'Área':'Texto';
    if(a.type==='angleDimension'){
      const g=angleDimensionGeom(a);el.inspector.innerHTML=`<div class="prop-section"><div class="prop-head">COTA ANGULAR</div><div class="prop-body"><div class="field"><label>Ángulo</label><input value="${angleDimensionLabel(a,g)}" readonly></div><div class="field"><label>Radio arco (m)</label><input id="annRadius" type="number" min="0.01" step="0.05" value="${fmt(g.radius,3)}"></div><div class="field"><label>Texto</label><input id="annText" value="${esc(a.textOverride||'')}" placeholder="Automático"></div><div class="prop-buttons"><button class="prop-btn" id="annBack">Cerrar</button><button class="prop-btn danger" id="annDelete">Eliminar</button></div><p class="drag-hint">El arco, su valor y la posición impresa comparten la misma geometría técnica.</p></div></div>`;$('#annRadius').addEventListener('input',e=>{a.radius=Math.max(.01,+e.target.value||g.radius);renderDrawing();renderSheet();});$('#annText').addEventListener('input',e=>{a.textOverride=e.target.value;renderDrawing();renderSheet();});
    }else if(a.type==='dimension'){
      const g=dimensionGeom(a),members=a.chainId?d.annotations.filter(x=>x.type==='dimension'&&x.chainId===a.chainId):[a];el.inspector.innerHTML=`<div class="prop-section"><div class="prop-head">${a.chainId?'CADENA DE COTAS':`COTA ${kindName.toUpperCase()}`}</div><div class="prop-body"><div class="field"><label>Medida</label><input value="${dimensionLabel(a,g)}" readonly></div>${a.chainId?`<div class="field"><label>Tramos</label><input value="${members.length}" readonly></div>`:''}<div class="field"><label>Dirección</label><input value="${a.axis==='H'?'Horizontal':a.axis==='V'?'Vertical':'Libre'}" readonly></div><div class="field"><label>Desfase (m)</label><input id="annOffset" type="number" step="0.05" value="${a.offset}"></div><div class="field"><label>Texto tramo</label><input id="annText" value="${esc(a.textOverride||'')}" placeholder="Automático"></div><div class="prop-buttons"><button class="prop-btn" id="annBack">Cerrar</button><button class="prop-btn danger" id="annDelete">${a.chainId?'Eliminar cadena':'Eliminar'}</button></div><p class="drag-hint">${a.chainId?'Arrastra cualquier tramo o cambia el desfase para mover toda la cadena a la vez.':'Arrastra directamente la línea o el texto de la cota para cambiar su altura sin borrarla.'} En PDF el texto se imprime con altura nominal de ${DIMENSION_TEXT_MM.toFixed(1).replace('.',',')} mm.</p></div></div>`;$('#annOffset').addEventListener('input',e=>{const value=+e.target.value||0;for(const member of members)member.offset=value;renderDrawing();renderSheet();});$('#annText').addEventListener('input',e=>{a.textOverride=e.target.value;renderDrawing();renderSheet();});
    }else if(a.type==='area'){
  ...
  function renderViewportInspector(vp,sh){
    const d=state.drawings.find(x=>x.id===vp.drawingId),nts=isNonScaleDrawing(d),is3d=d?.kind==='threeD',label=viewportLabelPosition(vp);el.inspectorTitle.textContent='Viewport';
    const imageSection=is3d?`<div class="prop-section"><div class="prop-head">ENCUADRE DE IMAGEN 3D</div><div class="prop-body"><div class="field"><label>Ajuste</label><select id="vpImageFit"><option value="cover" ${vp.imageFit!=='contain'?'selected':''}>Rellenar · recortar</option><option value="contain" ${vp.imageFit==='contain'?'selected':''}>Encajar completa</option></select></div><div class="field"><label>Foco X (%)</label><input id="vpImageFocusX" type="number" min="0" max="100" step="1" value="${fmt(Number.isFinite(+vp.imageFocusX)?+vp.imageFocusX:50,0)}"></div><div class="field"><label>Foco Y (%)</label><input id="vpImageFocusY" type="number" min="0" max="100" step="1" value="${fmt(Number.isFinite(+vp.imageFocusY)?+vp.imageFocusY:50,0)}"></div><p class="drag-hint"><b>Rellenar</b> mantiene la vista 3D grande y recorta el excedente. Los focos desplazan la zona visible.</p></div></div>`:'';
    el.inspector.innerHTML=`<div class="prop-section"><div class="prop-head">VISTA EN PLANO</div><div class="prop-body"><div class="field"><label>Vista</label><input value="${esc(d?.name||'—')}" readonly></div>${nts?`<div class="field"><label>Escala</label><input value="${d?.kind==='areaSchedule'?'TABLA':'NTS · Sin escala'}" readonly></div>`:`<div class="field"><label>Escala</label><select id="vpScale">${[...new Set([...standardScales,vp.scale])].sort((a,b)=>a-b).map(s=>`<option value="${s}" ${+s===+vp.scale?'selected':''}>1:${s}</option>`).join('')}</select></div>`}<div class="field"><label>X (mm)</label><input id="vpX" type="number" step="1" value="${fmt(vp.x,1)}"></div><div class="field"><label>Y (mm)</label><input id="vpY" type="number" step="1" value="${fmt(vp.y,1)}"></div><div class="field"><label>Ancho (mm)</label><input id="vpW" type="number" min="35" step="1" value="${fmt(vp.w,1)}"></div><div class="field"><label>Alto (mm)</label><input id="vpH" type="number" min="28" step="1" value="${fmt(vp.h,1)}"></div><div class="prop-buttons"><button class="prop-btn" id="vpCenter" ${nts?'disabled':''}>Recentrar</button><button class="prop-btn danger" id="vpDelete">Quitar</button></div><p class="drag-hint">Usa los tiradores centrales de izquierda, derecha, arriba y abajo para recortar cada borde de forma independiente.</p></div></div>${imageSection}<div class="prop-section"><div class="prop-head">TÍTULO Y ESCALA</div><div class="prop-body"><div class="field"><label>X (mm)</label><input id="vpLabelX" type="number" step="1" value="${fmt(label.x,1)}"></div><div class="field"><label>Y (mm)</label><input id="vpLabelY" type="number" step="1" value="${fmt(label.y,1)}"></div><div class="prop-buttons"><button class="prop-btn" id="vpLabelReset">Restablecer posición</button></div><p class="drag-hint">También puedes arrastrar directamente el título y la escala sobre el plano. Se mueven juntos y su posición se conserva al guardar y exportar.</p></div></div>`;
    const num=(id,key,min=null)=>$(id)?.addEventListener('change',e=>{const value=parseFloat(e.target.value);if(Number.isFinite(value))vp[key]=min==null?value:Math.max(min,value);renderSheet();renderInspector();});num('#vpX','x');num('#vpY','y');num('#vpW','w',35);num('#vpH','h',28);const labelNum=(id,key,base)=>$(id)?.addEventListener('change',e=>{const value=parseFloat(e.target.value),[pw,ph]=sheetSize(sh);if(Number.isFinite(value))vp[key]=clamp(value,10,key==='labelDx'?pw-10:ph-10)-base();renderSheet();renderInspector();});labelNum('#vpLabelX','labelDx',()=>vp.x);labelNum('#vpLabelY','labelDy',()=>vp.y);$('#vpLabelReset')?.addEventListener('click',()=>{delete vp.labelDx;delete vp.labelDy;renderSheet();renderInspector();});$('#vpScale')?.addEventListener('change',e=>{vp.scale=+e.target.value;renderSheet();renderInspector();});$('#vpCenter')?.addEventListener('click',()=>{if(d&&!nts){vp.cx=d.bounds.cx;vp.cy=d.bounds.cy;renderSheet();}});$('#vpImageFit')?.addEventListener('change',e=>{vp.imageFit=e.target.value;renderSheet();renderInspector();});num('#vpImageFocusX','imageFocusX');num('#vpImageFocusY','imageFocusY');$('#vpDelete')?.addEventListener('click',()=>{sh.viewports=sh.viewports.filter(x=>x.id!==vp.id);state.selectedViewportId=null;renderSheet();renderInspector();toast('Vista retirada del plano');});
  }
  ...
  el.drawingSvg.addEventListener('pointermove',e=>{
    ...
    if(isAlignedDimensionTool(state.drawTool)){
    ...
    }else if(state.drawTool==='angle-dimension'){state.pendingAngleDimension||={vertex:null,a:null,b:null,cursor:null};const pending=state.pendingAngleDimension,snap=getSnap(raw,d);pending.cursor=snap.type?snap.p:raw;state.snapHover=snap;renderDrawing();}
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Escape'){if(!el.modalBackdrop.classList.contains('hidden'))closeModal();else if(state.dragAnnotation){endAnnotationDrag({});}else if(state.ui.pan){endDrawingPan();}else if(state.ui.sheetPan){endSheetPan();}else if(state.pendingDimension||state.pendingAngleDimension||state.pendingDimensionChain||state.pendingArea){state.pendingDimension=null;state.pendingAngleDimension=null;state.pendingDimensionChain=null;state.pendingArea=null;state.snapHover=null;renderDrawing();toast('Operación cancelada');}else if(state.selectedAnnotationId){state.selectedAnnotationId=null;renderDrawing();renderInspector();}else if(state.selectedIfc){clearIfcSelection().then(()=>{renderDrawing();renderInspector();refreshFragmentsAfterCamera();});}}
